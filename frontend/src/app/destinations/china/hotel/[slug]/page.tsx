...
import { chinaHotels } from "@/data/hotels/China/chinaHotels";
...
export async function generateStaticParams() {
  return chinaHotels.map((hotel) => ({
    slug: hotel.slug,
  }));
}

async function getHotel(slug: string) {
  return chinaHotels.find((h) => h.slug === slug);
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const resolvedParams = await params;
  const hotel = chinaHotels.find((h) => h.slug === resolvedParams.slug);
